...
export default function CreateCertificate({
  ...
  const initialFields = [
    { id: "", title: "", type: "Text Input", placeholder: "" },
  ];
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // ✅ Point 1: Removed console.log("Uploaded successfully")
    } catch (err) {
      // ✅ Point 1: Removed console.error
...
const handlePublish = async () => {
    ...
    if (fields.length > 0 && fields.some((field) => field.title.trim().length < 5)) {
      error("Each field title must be at least 5 characters");
      return;
    }
      ...
      <div className="relative w-full max-w-xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        >
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-slate-800 border-l-4 border-blue-600 pl-3 italic">
              General Information
            </h3>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <label
                  htmlFor="file-up"
                  className="px-4 py-2 bg-white border border-blue-200 text-blue-600 rounded-lg text-xs font-bold hover:bg-blue-50 cursor-pointer flex items-center gap-2"
                >
                  {isUploading ? (
          ...
          <div className="space-y-4 pt-4">
            ...
            {fields.map((field) => (
              <div
                key={field.id}
                className="p-5 border border-slate-100 bg-slate-50/30 rounded-2xl space-y-4 relative group hover:border-blue-100 transition-all shadow-sm"
              >
