...
const SigninForm = () => {
  ...
  const { mutate: loginUser, isLoading: isLoggingIn, error } = useLogin();
  ...
  const onSubmit = (formData: SigninFormData) => {
    ...
    loginUser(sendData, {
      onSuccess: (data: any) => {
        // Validate response shape to avoid runtime errors when server returns HTML or unexpected data
        if (!data || typeof data !== "object" || !data.token || !data.user) {
          console.error("Login onSuccess: unexpected response:", data);
          setErrorMsg("Unexpected server response. Please try again later.");
          return;
        }

        Cookies.set("access", data.token);
        Cookies.set("role", data.user?.role?.name || "user");
        ...
        {(errorMsg || error) && (
          <div className="flex items-start gap-3 bg-red-50 text-red-700 px-3 py-2 rounded-lg border border-red-200 mb-2">
            ...
            <p className="text-[10px] font-medium leading-relaxed break-words mt-1">
              {errorMsg || (typeof error === "string" ? error : error?.message) || "Something went wrong. Please try again."}
            </p>
          </div>
        )}
