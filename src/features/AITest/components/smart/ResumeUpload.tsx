...
import { FileUploadArea } from "@/features/AITest/components/dumb/FileUploadArea";
import { FilePreview } from "@/features/AITest/components/dumb/FilePreview";
...
const ResumeUpload = ({
  ...
  onUploadSuccess: (questions: any) => void;
}) => {
  const { postResumeAndGenerateQuestionsMutation } = useAITest();
  const {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    const response = await postResumeAndGenerateQuestionsMutationAsync({
      data: formData,
